...
const VisualMemoryGameProvider = ({ children, exerciseTitle }) => {
		...
		while (newGlowingCards.length < glowCount) {
			...
			if (newGlowingCards.length === 0 || randomIndex !== lastGlowingIndex) {
				if (
					...
					newGlowingCards[newGlowingCards.length - sequenceCount - 1] === randomIndex
				) {
					continue;
				}
				...
				switch (promptType) {
					...
					case 'startGame':
						if (sequenceCount === 0) {
							...
							setSequenceCount((prevSequenceCount) => prevSequenceCount - 1);
						}
	...
	const startGame = () => {
		...
		const speed = levels.find((level) => level.levelObj === currentLevel)?.speed || 1000;
		...
		if (selectedCards.length === glowingCards.length - 1 && !isMatch) {
			...
			const isMatching =
				level >= 6 && sequenceCount === 0 // Check if the current level is less than 6
					? selectedCards.every(
							(selectedCard, index) =>
								selectedCard === glowingCards[glowingCards.length - 1 - index]
					  )
					: selectedCards.every((selectedCard, index) => selectedCard === glowingCards[index]);
