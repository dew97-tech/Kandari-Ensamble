// wordbox.js
const Right_Order_Game_Questions = [
   {
      exercise_id: 1,
      data: [
         {
...
export default function handler(req, res) {
   try {
      if (req.method === "GET") {
         const { id } = req.query;
         const exercise = Right_Order_Game_Questions.find((exercise) => exercise.exercise_id === parseInt(id));
         console.log("Fetching Right Order Exercise...");
         res.status(200).json(exercise);
         console.log("Returning Right Order Exercise as JSON...");
      } else {
      ...
      console.error(error);
      res.status(500).json({
         message: "Failed to fetch Right_Order_Game_Questions",
      });
   }
}
