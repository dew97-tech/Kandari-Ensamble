      ...
      <>
         <div className='d-flex align-items-center justify-content-center mx-3 mb-2'>
            {options.map((option, index) => {
               let buttonClass = "btn btn-md buff-text-color border-1 px-4 py-2 mx-2 shadow-sm fs-4";

               if (isSelectionMade) {
                  if (selectedOption === option.text) {
                     buttonClass += isCorrect
                        ? "btn btn-md light-green border-success fw-bolder fs-2"
                        : "btn btn-md bittersweet buff-text-color border-danger fw-bolder fs-2";
                  } else {
                     buttonClass += isCorrect
                        ? "btn btn-md bittersweet buff-text-color border-danger fw-bolder fs-2"
                        : "btn btn-md light-green border-success fw-bolder fs-2";
